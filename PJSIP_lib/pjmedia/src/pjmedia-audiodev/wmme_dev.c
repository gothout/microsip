					...
					unsigned buffer_count)
{
    ...
    */
    wmme_strm->hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
    if (NULL == wmme_strm->hEvent) {
	PJ_LOG(4, (THIS_FILE, "init_capture_stream CreateEvent failed"));
	return pj_get_os_error();
    }
    ...
    mr = waveInOpen(&wmme_strm->hWave.In, 
		    ...
		    &wfx, (DWORD_PTR)wmme_strm->hEvent, 0, flag);
    if (mr != MMSYSERR_NOERROR) {
	PJ_LOG(4, (THIS_FILE, "init_capture_stream waveInOpen failed (%d, %d, %d)",
		wmme_strm->hWave.In,
		wf->dev_info[prm->rec_id].deviceId,
		flag
		));
	...
	mr = waveInPrepareHeader(wmme_strm->hWave.In, 
				 ...
				 sizeof(WAVEHDR));
	if (mr != MMSYSERR_NOERROR) {
		PJ_LOG(4, (THIS_FILE, "init_capture_stream waveInPrepareHeader failed"));
	    return PJMEDIA_AUDIODEV_ERRNO_FROM_WMME_IN(mr);
	}
	mr = waveInAddBuffer(wmme_strm->hWave.In, &(wmme_strm->WaveHdr[i]), 
			     sizeof(WAVEHDR));
	if (mr != MMSYSERR_NOERROR) {
		PJ_LOG(4, (THIS_FILE, "init_capture_stream waveInAddBuffer failed"));
	    return PJMEDIA_AUDIODEV_ERRNO_FROM_WMME_IN(mr);
	}
    }
					 ...
					 pjmedia_aud_stream **p_aud_strm)
{
	...
	if (status != PJ_SUCCESS) {
		PJ_LOG(4, (THIS_FILE, "init_capture_stream failed with code %d", status));
	    stream_destroy(&strm->base);
	    return status;
	}
    }
