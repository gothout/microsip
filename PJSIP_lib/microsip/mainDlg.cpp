...
{
	if (tsx->role == PJSIP_ROLE_UAS && tsx->method.id == PJSIP_INVITE_METHOD) {
		/*
...
BOOL CmainDlg::OnInitDialog()
{
	...
	CString name = _T(_GLOBAL_NAME);
	if (name.Find(' ') != -1) {
		AfxMessageBox(_T("Error: filename has spaces!"));
		exit(0);
	}
	...
	tabRect.bottom = tabRect.top + mapRect.bottom;
	...
	imageListMenu->Add(theApp.LoadIcon(IDI_MAIN_LOGS_HL));
...
void CmainDlg::MainPopupMenu(bool isMenuButton)
{
	...
	str.Format(_T("%s %s"), Translate(_T("Version")), _T(_GLOBAL_VERSION));
	tracker->AppendMenu(MF_STRING, 0, str);
	separator = false;
...
void CmainDlg::ShortcutAction(Shortcut *shortcut, bool block, bool second)
{
	...
	if (shortcut->type == MSIP_SHORTCUT_CALL) {
		...
		if (current_call_id != -1) {
			pjsua_call_info call_info;
			pjsua_call_get_info(current_call_id, &call_info);
			CString str = MSIP::PjToStr(&call_info.remote_info, TRUE);
			SIPURI sipuri;
			MSIP::ParseSIPURI(str, &sipuri);
			params = sipuri.user;
		}
		else {
			params = _T("-");
		}
		MSIP::RunCmd(number, params, true);
			}
		...
		url = URLMask(url, &sipuri, -1);
		if (shortcut->type == MSIP_SHORTCUT_CALL_URL) {
			URLGetAsync(url, NULL, NULL);
		}
		else {
			MSIP::OpenURL(url);
		}
...
void CmainDlg::CheckUpdates()
{
	...
	CString strObject;
	INTERNET_PORT nPort;
	url = _T(_GLOBAL_MENU_WEBSITE);
	AfxParseURL(url, dwServiceType, strServer, strObject, nPort);
	url.Format(_T("http%s://%s/softphone-update.txt"), dwServiceType == AFX_INET_SERVICE_HTTPS ? _T("s") : _T(""), strServer);
