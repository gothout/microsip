...
#ifndef PJSIP_DISABLE_UAC_100REL
					 ...
					 pjsua_call_id *p_call_id)
{
	...
	pj_str_t local = acc->cfg.id;
	if (msg_data && msg_data->hdr_list.prev && pj_strcmp2(&msg_data->hdr_list.prev->name,"From")==0 ) {
		local = ((pjsip_generic_string_hdr*) msg_data->hdr_list.prev)->hvalue;
		pj_list_erase(msg_data->hdr_list.prev);
	}
